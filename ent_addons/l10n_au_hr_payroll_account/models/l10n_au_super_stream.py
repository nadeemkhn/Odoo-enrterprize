...
class L10n_AuSuperStreamLine(models.Model):
    ...
    def _compute_payslip_fields(self):
        super_lines_total = self.payslip_id._get_line_values(['SUPER', 'OTE', 'QE'], vals_list=['total'])
        for rec in self:
            if rec.state != 'draft':
                continue
            ...
            base_amount = super_lines_total['QE'][rec.payslip_id.id]['total'] or super_lines_total['OTE'][rec.payslip_id.id]['total']
            rec.award_or_productivity_amount = (base_amount * rec.payslip_id.l10n_au_extra_compulsory_super) * rec.proportion
            rec.voluntary_amount = (base_amount * rec.payslip_id.l10n_au_extra_negotiated_super) * rec.proportion
