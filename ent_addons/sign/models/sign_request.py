            ...
            if sign_request_item.role_id in sign_values_by_role:
                ...
                for key, value in sign_items.items():
                    ...
                    if value.get("auto_field"):
                        auto_value = sign_request_item._get_auto_field_value({
                            "id": value.get("type_id"),
                            "auto_field": value.get("auto_field")
                        })

                        # Only set the value if the auto_value is not empty, to avoid overwriting existing values with empty ones.
                        if auto_value not in ('', None, False):
                            corrected_dict[key] = auto_value
