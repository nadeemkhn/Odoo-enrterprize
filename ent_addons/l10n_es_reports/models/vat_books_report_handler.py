            ...
            for config in sheet_configs:
                for move_idx in config['line_vals']:
                    for line_vals in config['line_vals'][move_idx].values():
                        current_row = row_trackers[config['type']]
                        for col_idx, field in enumerate(config['fields']):
                            if field in FORMAT_NEEDED_FIELDS and line_vals[field] and options.get('number_format'):
                                config['sheet'].write(current_row, col_idx, line_vals[field], options.get('number_format'))
                            else:
                                config['sheet'].write(current_row, col_idx, line_vals[field])
                        row_trackers[config['type']] += 1
