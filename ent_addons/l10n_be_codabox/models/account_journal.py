        ...
        for coda in codas:
            try:
                ...
                for currency, account_number, __ in bank_statement_file_data:
                    ...
                    candidates = acc_journal_map.get(sanitize_account_number(account_number), self.env['account.journal'])
                    journal = (
                        candidates.filtered(lambda j: j.currency_id.name == currency)[:1]
                        or candidates.filtered(lambda j: not j.currency_id)[:1]
                    )
                    if journal:
